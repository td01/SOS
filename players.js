...
var PLAYERS = {
  ...
  ARG_Messi:     { full:'Lionel Messi', age:38, pos:'Forward', club:'Inter Miami', foot:'Left', caps:188, intlGoals:120, wcGoals:16, wcGoalsPre2026:13, wcApps:6, height:'170cm', number:10, stats:{goals:120,assists:58,wcGoals:16,clubGoals25_26:12}, facts:['Scored a hat-trick vs Algeria (Jun 17, 2026) — his first ever at a World Cup','16 World Cup goals — tied with Miroslav Klose for the all-time record','Won Qatar 2022: 7 goals, Golden Ball, player of tournament','8× Ballon d\'Or winner — most in history','Record 6th World Cup — joint record with Cristiano Ronaldo','Argentina\'s all-time top scorer with 120 international goals'], quote:'"This is probably my last World Cup. I want to savour every moment."' },
...
// Matches a name as it comes from the live API (often abbreviated, e.g.
// "L. Messi") against a profile's full name (e.g. "Lionel Messi"). Used to
// look up a specific player's live tournament stats by name rather than
// by a stable ID, since the live API doesn't expose one we store locally.
function playerNamesMatch(apiName, profileFullName) {
  var apiParts = apiName.replace(/['']/g,"'").split(' ');
  var profileParts = profileFullName.replace(/['']/g,"'").split(' ');
  var apiLast = apiParts[apiParts.length - 1].replace(/[^a-zA-ZÀ-ÿ]/g,'').toLowerCase();
  var profileLast = profileParts[profileParts.length - 1].replace(/[^a-zA-ZÀ-ÿ]/g,'').toLowerCase();
  if (apiLast !== profileLast) return false;

  var apiFirst = apiParts[0].replace(/[^a-zA-ZÀ-ÿ.]/g,'').replace(/\.$/, '');
  var profileFirst = profileParts[0].replace(/[^a-zA-ZÀ-ÿ]/g,'');
  return firstNameMatches(apiFirst, profileFirst);
}
